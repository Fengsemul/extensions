...
(() => {
    ...
    async function refreshStatus() {
        ...
        elements.manualLabelCount
            .textContent =
            formatCount(manual.labels);

        elements.manualExactCount
            .textContent =
            formatCount(manual.exactHosts);
        ...
        elements.activeGeneration
            .textContent =
            status.counts.activeGeneration ===
                null
                ? "None"
                : String(
                    status.counts
        ...
        elements.stagingGeneration
            .textContent =
            status.stagingGeneration === null
                ? "None"
                : String(
    ...
    function renderGenerations(records) {
        elements.generationRows
            .replaceChildren();
        ...
        for (const record of records) {
            ...
            if (
                ...
                button.className = "danger";
                               button.dataset.generationDelete =
            ...
            elements.generationRows.appendChild(row);
        }
    }
    function renderManualImports(records) {
        elements.manualImportRows.replaceChildren();
        if (records.length === 0) {
            ...
            cell.colSpan = 9;
            elements.manualImportRows.appendChild(row);
            return;
        }
        for (const record of records) {
            const row = document.createElement("tr");
            ...
            createCell(
                row,
                formatCount(record.validRules),
                "numeric"
            );
            createCell(
                row,
                formatCount(record.uniqueRules),
                "numeric"
            );
            createCell(
                row,
                formatCount(record.rejectedRules),
                "numeric"
            );
            createCell(
                row,
    ...
    function updateManualSelection() {
        elements.manualFileSelection.textContent =
            describeFile(
    ...
    async function importProductionPackage() {
        ...
        try {
            ...
            setStatus(
                elements.productionStatus,
                "Verifying and activating the new generation..."
            );
            await send({
                type:
                    "activateProductionGeneration",
                generation
            });
            elements.productionProgress.value =
                100;
            setStatus(
                elements.productionStatus,
                `Production package activated as generation ${generation}.`,
                "success"
            );
            await refreshStatus();
        } catch (error) {
    ...
    async function deleteGeneration(
        ...
        const confirmed =
            window.confirm(
                `Delete production generation ${generation}?`
            );
        if (!confirmed) {
            return;
        }
        ...
        try {
            ...
            setStatus(
                elements.actionStatus,
                `Generation ${generation} was deleted.`,
                "success"
            );
            await refreshStatus();
        } catch (error) {
    ...
    async function removeManualImport(
        ...
        const confirmed =
            window.confirm(
                `Remove manual import "${record.fileName}" from ${CATEGORY_NAMES[record.category] || record.category}?`
            );
        if (!confirmed) {
            return;
        }
        ...
        try {
            ...
            setStatus(
                elements.actionStatus,
                `Removed ${response.result.removedLinks.toLocaleString()} ownership links and ` +
                    `${response.result.removedRules.toLocaleString()} unreferenced rules.`,
                "success"
            );
            await refreshStatus();
        } catch (error) {
    ...
    async function deleteDatabase() {
        ...
        const confirmed =
            window.confirm(
                "Delete the entire LeanSERP Filter v2 database? " +
                    "All production generations and manual imports will be removed."
            );
        if (!confirmed) {
            return;
        }
        ...
        try {
            ...
            setStatus(
                elements.actionStatus,
                "The v2 database was deleted. Reloading manager...",
                "success"
            );
            window.setTimeout(
                () => {
    ...
    elements.openOptions.addEventListener(
        "click",
        () => {
